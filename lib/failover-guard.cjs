...
const WRITE_HANDLERS=new Set([
  ...
  'process-payment-report','send-receipt','resend-receipt','receipt-recovery-background','mkj-access','access-mode','access-auto-sync','bcv-refresh'
]);
...
function validateBasePair(env=process.env){
  const dataEnv=clean(env.VLA_DATA_ENVIRONMENT||lock.defaultDataEnvironment).toLowerCase();
...
function authorize(handler,env=process.env){
  ...
  if(!isWrite(name))return{allowed:true,write:false,mode:clean(env.VLA_FAILOVER_WRITE_MODE||lock.writesDefault).toLowerCase(),...pair};
  const mode=clean(env.VLA_FAILOVER_WRITE_MODE||lock.writesDefault).toLowerCase();
...
module.exports={WRITE_HANDLERS,HARD_BLOCKED,WHATSAPP_PREFIX,clean,activationFingerprint,isWrite,validateBasePair,authorize};
