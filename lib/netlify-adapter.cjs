...
async function runHandler(name,handler,req,res){try{authorize(name,process.env);ensureRuntimeUrl(req);const event=eventFromRequest(req);const result=await handler(event,{});return sendNetlifyResponse(res,result);}catch(error){console.error('VLA_FAILOVER_HANDLER_ERROR',name,error?.code||'',error?.message||error);return errorResponse(res,error);}}
