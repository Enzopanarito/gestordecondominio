...
const MIXED_WRITE_HANDLERS=new Set(['admin-plant','public-plant']);
...
function methodName(method){return clean(method||'').toUpperCase();}
...
function isWrite(handler,method=''){
  const name=clean(handler),verb=methodName(method);
  if(WRITE_HANDLERS.has(name))return true;
  if(!MIXED_WRITE_HANDLERS.has(name))return false;
  if(!verb)return true;
...
function authorize(handler,env=process.env,method=''){
  ...
  if(!isWrite(name,method))return{allowed:true,write:false,mode,...pair};
...
module.exports={WRITE_HANDLERS,MIXED_WRITE_HANDLERS,HARD_BLOCKED,WHATSAPP_PREFIX,clean,methodName,activationFingerprint,isWrite,effectiveDataEnvironment,effectiveWriteMode,effectiveBaseId,validateBasePair,materializeRuntimeDefaults,realStagingReadsEnabled,authorize};
